...
no single day can show. Two-phase by design:
  ...
  (their rule: no enumeration; examples become walls).
...
Their verdict is ADOPTION, not fact-checking: verification proves the numbers,
they rule on the meaning. adopted = may quietly shape decisions; dismissed =
never resurfaces; watching = keep testing. It learns about them; it never
manages them.
...
class PostgresWatcherRepository:
        ...
        with self._db.connect() as conn:
            with conn.cursor() as cur:
                if verified:
                    ...
                    # Adopted is untouched: their verdict outranks a noisy week.
                    cur.execute(
                        """
                        UPDATE watcher_patterns
    ...
    def find_by_words(self, user_id: UUID, words: str) -> list[dict]:
        """Patterns whose hypothesis contains their words (case-insensitive) —
...
hypothesis, and never re-propose anything they dismissed, however reworded.
...
- {{"type": "theme_recurrence", "theme": "<short phrase>", "window_days": 60,
   "min_items": 5}} — does a meaning keep recurring in what they file?
...
class Watcher:
    ...
    def _garden_summary(self, user_id: UUID, frame: dict[date, dict]) -> str:
        ...
        it (their call: it sees everything). Numbers in the daily lines; the mind's
        garden (seeds, efforts, captures), the runs as they felt them, and the
    ...
    def _focus_lines(self, user_id: UUID) -> str:
        """The mind's garden: seeds (curiosities), efforts (what they're building),
    ...
    def _runs_lines(self, user_id: UUID) -> str:
        """Runs as THEY experienced them — the notes carry their annotations
        ('social run', 'loved the intervals')."""
        runs = self._runs.recent_runs(user_id, limit=30)
        if not runs:
            return ""
        lines = ["RUNS (their words in the notes):"]
        for r in runs:
    ...
    def respond(self, user_id: UUID, pattern_id: UUID, verdict: str,
                note: str | None) -> dict | None:
        """Record their verdict and refresh the vault window."""
        ...
        "properties": {
            "pattern": {
                ...
                "description": (
                    ...
                    "one, OR a distinctive phrase from the hypothesis in their words "
                    "('meds', 'body battery on run days') — they may be reacting to "
                    "the Watcher page in their vault, which shows patterns you can't "
                    "see in context yet."
                ),
            },
            "verdict": {"type": "string", "enum": ["adopted", "dismissed", "watching"]},
            "note": {"type": "string", "description": "Their reasoning, briefly, if they gave one."},
        },

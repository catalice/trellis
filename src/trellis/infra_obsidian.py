...
class ObsidianVault:
        ...
        try:
            ...
            if upcoming_reminders:
                ...
                for r in upcoming_reminders:
                    ...
                    icon = "🗣️" if getattr(r, "kind", "remind") == "check_in" else "🔔"
                    parts.append(f"- {icon} {r.label} — {self._fmt(r.remind_at)}{recur}")
                parts.append("")
            if completed:
    ...
    def _render_tracking_days(self, states, events, period_start,
        ...
        for day in days:
            ...
            if g_bits and sleeps:
                parts.append(f"**Sleep:** {' '.join(g_bits)} (Garmin) · their report: {', '.join(sleeps)}")
            elif g_bits:
    ...
    def _update_daily_properties(self, user_id: UUID, day: date) -> None:
        ...
        try:
            ...
            # Sleep: Garmin is the source of truth when synced (their call, 12 Aug
            # — hours, score, and the stage splits); their own report is the
    ...
    def _write_training_week(self, week: list, runs_by_date: dict,
        ...
        # to a human; "2026-W33" means nothing (their words, 12 Aug).
        lines = [

-- Restore the Garmin health tables. The read + sync CODE for daily health,
-- activities and self-reports survived the second-brain trim, but the TABLES were
-- archived — so latest_daily_health / the garmin-sync had nothing to read/write.
-- The running coach now reads recent health/readiness, so the tables are needed
-- again. Schema matches infra_tracking's repositories (restored from _archive/005).
